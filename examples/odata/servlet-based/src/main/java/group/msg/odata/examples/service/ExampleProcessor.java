...
import group.msg.odata.examples.data.ExampleDataStore;
...
import static group.msg.odata.examples.edm.ExampleEdmProvider.ENTITY_SET_NAME_CARS;
import static group.msg.odata.examples.edm.ExampleEdmProvider.ENTITY_SET_NAME_MANUFACTURERS;

public class ExampleProcessor extends ODataSingleProcessor {
	private final ExampleDataStore dataStore;
		...
		if (uriInfo.getNavigationSegments().isEmpty()) {
			...
			if (ENTITY_SET_NAME_CARS.equals(entitySet.getName())) {
				return EntityProvider
						.writeFeed(contentType, entitySet, dataStore.getCars(), EntityProviderWriteProperties
								.serviceRoot(getContext().getPathInfo().getServiceRoot()).build());
			} else if (ENTITY_SET_NAME_MANUFACTURERS.equals(entitySet.getName())) {
				return EntityProvider
						.writeFeed(contentType, entitySet, dataStore.getManufacturers(), EntityProviderWriteProperties
								.serviceRoot(getContext().getPathInfo().getServiceRoot()).build());
			}
			...
			if (ENTITY_SET_NAME_CARS.equals(entitySet.getName())) {
				...
				return EntityProvider.writeFeed(contentType, entitySet, cars, EntityProviderWriteProperties
						.serviceRoot(getContext().getPathInfo().getServiceRoot()).build());
			}
